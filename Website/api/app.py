...
def generate_combinations(ingredients_list):
    ans = dict()
    ans[0] = []
    result = requests.get('https://www.themealdb.com/api/json/v1/1/search.php?s='+ ingredients_list)
    meal = result.json()['meals']
    for m in meal:
        title = m['strMeal']
        img = m['strMealThumb']
        ins = m['strInstructions']
        src = m['strSource']
        ingr = []
        for i in range(1,6):
            ingr.append(m['strIngredient'+ str(i)])

        ans[0].append(
            {'author':title, 'title': ingr,'content': ins, 'date_posted':  img,'src':src, 'isFavorite': "false",
             'classname': "heart"})
...
def home():
    ...
    ans = generate_combinations(ingredients_list)
